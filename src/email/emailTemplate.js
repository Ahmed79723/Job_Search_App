export const emailTemplate = (token, name, otp) => {
  // check received email token bearer
  const [bearer, core] = token.split(" ");
  if (bearer === "emailConfirmToken") {
    // sending email token to the end point and sending it in it's params
    // along with user name to display it in email
    // also sending the otp to the otp end point link
    return `
...
<table border="0" cellpadding="0" cellspacing="0" width="100%" style="table-layout:fixed;background-color:#f9f9f9" id="bodyTable">
  ...
  res.status(404).json({ msg: "wrong token bearer" });
};
